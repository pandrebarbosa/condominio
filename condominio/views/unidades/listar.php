...
<div class="panel panel-default">
  ...
  <div class="panel-body">
		<div class="row">
		  ...
		  <div class="col-md-5">
		  	<input type="text" class="form-control" id="criterio" placeholder="Digite o numero da unidade">
		  </div>
		  <div class="col-md-5">
				<button class="btn btn-info" id="btn-buscar">Buscar</button>
				<button class="btn btn-warning" id="btn-reset">Limpar</button>
				<button class="btn btn-primary" id="btn-novo">Nova Unidade</button>		  
		  </div>
...
var montaResultadoTabela = function(result, header, divResultado, pagina, criterio, maximo, tipo_unidade) {
	...
	$.each(result, function (key, val) {
		...
		if(i>1){
			...
			tabela += "<td>";
			tabela += "<a class='btn btn-primary btn-sm' title='Detalhar' href='default.php?ido=<?php echo base64_encode("unidades-manter")?>&co_unidade=" + val.co_unidade + "&nu_numero=" + val.nu_numero + "&no_torre=" + val.no_torre + "'><i class='glyphicon glyphicon-pencil'></i></a>";
			tabela += "</td>";
			tabela += "</tr>";
		}else{
			total = val.total;
...
$( "#btn-novo" ).click(function() {
	document.location.href='?ido=<?php echo base64_encode("unidades-manter")?>';
});	
</script>
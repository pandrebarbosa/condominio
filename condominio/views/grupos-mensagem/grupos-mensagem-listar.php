  ...
  <div class="panel-body">
		<div class="row">
		  ...
		  <div class="col-md-5">
				...
				<button class="btn btn-primary" id="btn-novo">Novo Grupo</button>		  
		  </div>
...
var montaResultadoTabela = function(result, header, divResultado, pagina, criterio, maximo) {
	...
	$.each(result, function (key, val) {
		...
		if(i>1){
			...
			tabela += "<td>";
			tabela += "<a class='btn btn-primary btn-sm' title='Detalhar' href='default.php?ido=grupos-mensagem-manter&co_grupo=" + val.co_grupo+"'><i class='glyphicon glyphicon-pencil'></i></a>";
			tabela += "</td>";
			tabela += "</tr>";
		}else{
			total = val.total;
...
var grid = function(maximo, pagina, criterio) {		
	$.ajax({
	  ...
	  dataType: "json",
	  loading: true,
	  url: "services/grupos-mensagem/grupos-listar.php",
	  data: { maximo: maximo, pagina: pagina, criterio: criterio }
	}).done(function( data ) {		
		var header = ["Nome","Descrição","Método PHP"];
...
$( "#btn-novo" ).click(function() {
	document.location.href='?ido=grupos-mensagem-manter';
});	
</script>
...
<div class="panel panel-default">
  ...
  <div class="panel-body">
		<div class="row">
		  <div class="col-md-5">
		  	<input type="text" class="form-control" id="criterio" placeholder="Busca de tipo de morador">
		  </div>
		  <div class="col-md-5">
				<button class="btn btn-info" id="btn-buscar">Buscar</button>
				<button class="btn btn-warning" id="btn-reset">Limpar</button>
				<button class="btn btn-primary" id="btn-novo">Novo tipo de morador</button>		  
		  </div>
...
var montaResultadoTabela = function(result, header, divResultado, pagina, criterio, maximo) {
	...
	$.each(result, function (key, val) {
		...
		if(i>1){
			...
			tabela += "<td>";
			tabela += "<a class='btn btn-primary btn-sm' title='Detalhar' href='default.php?ido=<?php echo base64_encode("tipos-de-moradores-manter")?>&co_tipo_morador=" + val.co_tipo_morador+"'><i class='glyphicon glyphicon-pencil'></i></a>";
			tabela += "</td>";
			tabela += "</tr>";
		}else{
			total = val.total;
...
var grid = function(maximo, pagina, criterio) {		
	$.ajax({
	  ...
	  dataType: "json",
	  loading: true,
	  url: "services/tipos-de-moradores/listar.php",
...
$( "#btn-novo" ).click(function() {
	document.location.href='?ido=<?php echo base64_encode("tipos-de-moradores-manter")?>';
});	
</script>
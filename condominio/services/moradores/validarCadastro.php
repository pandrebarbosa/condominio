...
if( $Morador->getCoTipoMorador() == NULL ){
	$resultado =  array("id" => null, "tipo" => "erro", "msg" => "Tipo de morador não identificado!");
	echo json_encode($resultado);
	exit;
}
if( $Morador->getDtInicio() == NULL ){
	$resultado =  array("id" => null, "tipo" => "erro", "msg" => "Data de início não preenchido!");
	echo json_encode($resultado);
	exit;
}
if( $Pessoa->getNoPessoa() == NULL ){
	$resultado =  array("id" => null, "tipo" => "erro", "msg" => "Nome do morador não preenchido!");
	echo json_encode($resultado);
	exit;
}
...
if( $Morador->getCoTipoMorador()==1 || $Morador->getCoTipoMorador()==4 ){
	if ($Pessoa->getNuCpf() == "" ||
		...
		$resultado =  array("id" => null, "tipo" => "erro", "msg" => "CPF do <b>proprietário ou locatário</b> inválido!");
		echo json_encode($resultado);
		exit;
	}
}
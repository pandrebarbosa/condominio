...
if( empty( $no_pessoa )){
	$resultado =  array("id" => null, "tipo" => "erro", "msg" => "Nome do usuário não preenchido!");
...
if( empty( $ds_unidade )){
	$resultado =  array("id" => null, "tipo" => "erro", "msg" => "Campo unidade(s) não preenchido!");
	echo json_encode($resultado);
	exit;
}
...
if($email){
	$resultado =  array("tipo" => "sucesso", "msg" => "Solicitação enviada com sucesso. Aguarde instruções por email.");
}else{
	$resultado =  array("tipo" => "erro", "msg" => "Erro ao enviar email.");
}

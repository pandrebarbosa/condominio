...
class ListaCorreiosGeralRelatorio extends FPDF{
	...
	function Header(){
	    ...
	    $this->Cell(190,5,utf8_decode('Relatório Geral de Correspondências'),0,0,'C');
	    // Line break
	    $this->Ln(15);	    
	}
	...
	function montarTabela($header, $data){
		...
		$this->SetFont('','B',10);
		//Header
		$this->SetWidths(array(30, 90, 35, 53, 30, 42));
		for($i=0;$i<count($header);$i++)
			...
			foreach($data as $row)
			{
				...
				$valores = array($row['dt_hr_chegada'],
						strtoupper($campoItem),
						utf8_decode(strtoupper($row['unidade'])),
						utf8_decode(strtoupper(toolBox::retornaNomeSobrenome($row['recebedor']) . ' - ' . $row['no_cargo_funcionario'] )),
						utf8_decode($row['dt_hr_retirada']),
						utf8_decode($row['ds_retirada'])
				);
				$this->Row($valores);
